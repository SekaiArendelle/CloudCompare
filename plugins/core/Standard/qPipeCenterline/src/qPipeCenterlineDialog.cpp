...
{
	if (m_radiusEstimateSpinBox) m_radiusEstimateSpinBox->setValue(radius);
}

void qPipeCenterlineDialog::setVoxelSize(double size)
{
	if (m_voxelSizeSpinBox) m_voxelSizeSpinBox->setValue(size);
}

void qPipeCenterlineDialog::setMinPointsPerSegment(int points)
{
	if (m_minPointsSpinBox) m_minPointsSpinBox->setValue(points);
}

void qPipeCenterlineDialog::setCurvatureThreshold(double threshold)
{
	if (m_curvatureThresholdSpinBox) m_curvatureThresholdSpinBox->setValue(threshold);
}

void qPipeCenterlineDialog::setDistanceThreshold(double threshold)
{
	if (m_distanceThresholdSpinBox) m_distanceThresholdSpinBox->setValue(threshold);
}

void qPipeCenterlineDialog::setBranchDetectionEnabled(bool enabled)
{
	if (m_branchDetectionCheckBox) m_branchDetectionCheckBox->setChecked(enabled);
}

void qPipeCenterlineDialog::setBranchAngleThreshold(double angle)
{
	if (m_branchAngleSpinBox) m_branchAngleSpinBox->setValue(angle);
}

void qPipeCenterlineDialog::setRANSACEnabled(bool enabled)
{
	if (m_ransacCheckBox) m_ransacCheckBox->setChecked(enabled);
}

void qPipeCenterlineDialog::setRANSACDistanceThreshold(double threshold)
{
	if (m_ransacDistanceSpinBox) m_ransacDistanceSpinBox->setValue(threshold);
}

void qPipeCenterlineDialog::setRANSACMaxIterations(int iterations)
{
	if (m_ransacIterationsSpinBox) m_ransacIterationsSpinBox->setValue(iterations);
}

void qPipeCenterlineDialog::setMLSREnabled(bool enabled)
{
	if (m_mlsrCheckBox) m_mlsrCheckBox->setChecked(enabled);
}

void qPipeCenterlineDialog::setMLSRSearchRadius(double radius)
{
	if (m_mlsrRadiusSpinBox) m_mlsrRadiusSpinBox->setValue(radius);
}